...
# Global dictionary to store active peers.
# Key: tuple (peer_ip, peer_port) where peer_port is the known listening port.
# Value: the socket object.
active_peers = {}
peers_lock = threading.Lock()

# Global event to signal shutdown to threads.
shutdown_event = threading.Event()

# Global variable to store our own listening port.
my_listen_port = None
...
def get_local_ip():
    ...
    try:
        # This doesn't need to be reachable; it's only used to obtain the local IP.
...
def send_mandatory_messages():
    ...
    for ip, port in mandatory_peers:
        ...
        # Adjust the message below as needed.
...
def main():
    ...
    # Send the mandatory messages to the specified IPs.
    ...
    while True:
        ...
        if choice == "1":
            ...
            send_message(target_ip, target_port, team_name+" : " +message)
